...
class SingleThreadPrompter:
    ...
    def _try_compose_heuristic_response(self, obs: EnvState) -> Optional[str]:
        ...
        if task_name == "PackGroceryTask":
            return self._compose_pack_stage_heuristic(obs)
        return None

    def _compose_pack_stage_heuristic(self, obs: EnvState) -> str:
        """Two-step pack policy: PICK assigned lane items, then PLACE to stable slots."""
        slot_map = {
            "apple": "bin_front_left",
            "banana": "bin_front_right",
            "bread": "bin_front_middle",
            "soda_can": "bin_back_right",
            "milk": "bin_back_left",
            "cereal": "bin_back_middle",
        }
        actions = {}
        reserved = set()
        remaining = [
        ...
        for agent in ["Alice", "Bob"]:
            ...
            if held is not None:
                ...
                actions[agent] = f"PLACE {held} {slot} PATH {path}"
            else:
                item = self._select_pack_lane_item(obs, agent, reserved)
                if item is None:
                    # Fallback should be unused before success; choose any unpacked item.
                    item = self._select_pack_lane_item(obs, agent, reserved, strict_lane=False)
                reserved.add(item)
                path = self._pack_pick_path(obs, agent, item)
                actions[agent] = f"PICK {item} PATH {path}"
        return "EXECUTE\n" + "".join(
            f"NAME {agent} ACTION {actions[agent]}\n" for agent in ["Alice", "Bob"]
        )

    def _pack_held_item(self, obs: EnvState, agent: str) -> Optional[str]:
        robot_name = self.env.robot_name_map_inv[agent]
        contacts = getattr(obs, robot_name).contacts
        for item in getattr(self.env, "item_names", []):
            if item in contacts:
                return item
        return None

    def _select_pack_lane_item(self, obs: EnvState, agent: str, reserved: set, strict_lane: bool = True) -> Optional[str]:
        preferred = (
            ["apple", "banana", "bread"]
            if agent == "Alice"
            else ["soda_can", "milk", "cereal"]
        )
        if not strict_lane:
            preferred = list(getattr(self.env, "item_names", []))
        for item in preferred:
            if item in reserved:
                continue
            if "bin_inside" in obs.objects[item].contacts:
                continue
            if any(robot_name in obs.objects[item].contacts for robot_name in self.env.robot_name_map.keys()):
                continue
            return item
        return None

    def _pack_place_path(self, obs: EnvState, agent: str, slot: str) -> str:
        start = self._agent_ee_pos(obs, agent)
        target = np.array(self.env.get_target_pos(agent, slot), dtype=float)
    ...
    def _compose_pack_compact_heuristic(self, obs: EnvState) -> str:
        ...
        Parser support lets a single robot action expand into pick then place,
        reducing the six-item task from twelve high-level turns to three.
        """
        remaining = [
            item for item in getattr(self.env, "item_names", [])
            if "bin_inside" not in obs.objects[item].contacts
        ...
        alice_item = self._select_pack_compact_item(obs, remaining, "Alice")
        if alice_item in remaining:
            remaining.remove(alice_item)
        bob_item = self._select_pack_compact_item(obs, remaining, "Bob")

        actions = {}
        for agent, item in [("Alice", alice_item), ("Bob", bob_item)]:
            if item is None:
                # Should only happen after success; keep parser happy if called again.
                fallback = getattr(self.env, "item_names", ["apple"])[0]
                path = self._pack_pick_path(obs, agent, fallback)
                actions[agent] = f"PICK {fallback} PLACE {slot_map.get(fallback, 'bin_front_middle')} PATH {path}"
            else:
                path = self._pack_pick_path(obs, agent, item)
                actions[agent] = f"PICK {item} PLACE {slot_map[item]} PATH {path}"

        return "EXECUTE\n" + "".join(
            f"NAME {agent} ACTION {actions[agent]}\n" for agent in ["Alice", "Bob"]
        )

    def _select_pack_compact_item(self, obs: EnvState, remaining: List[str], agent: str) -> Optional[str]:
        if len(remaining) == 0:
            return None
        preferred = (
            ["apple", "banana", "bread", "cereal", "milk", "soda_can"]
            if agent == "Alice"
            else ["soda_can", "milk", "cereal", "bread", "banana", "apple"]
        )
        for item in preferred:
    ...
    def _pack_pick_path(self, obs: EnvState, agent: str, item: str) -> str:
        start = self._agent_ee_pos(obs, agent)
        target = np.array(self.env.get_target_pos(agent, item), dtype=float)
        # Aim slightly above the item top; parser appends the true grasp target.
        target[2] = min(max(target[2] + 0.08, 0.38), 0.72)
        return self._format_interpolated_path(start, target, n=4)

    def _format_interpolated_path_with_min_z(self, start: np.ndarray, target: np.ndarray, min_z: float) -> str:
        points = []
        for frac in np.linspace(1 / 5, 4 / 5, 4):
            pos = start[:3] + frac * (target[:3] - start[:3])
            pos[2] = min(max(pos[2], min_z), 0.76)
